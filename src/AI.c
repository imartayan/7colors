...
int minimax(pstate state, bool maximizing_Player, char player){
  ...
  if(partie_finie(new_state->score1, new_state->score2)){
    int score;
    if(player == PLAYER1)
      score = state->score1;
    else
  ...
  for(int i = 0; i < NB_COLORS; i++){
    ...
    if(state->curr_player == PLAYER1)
      ...
      new_state->curr_player = PLAYER1;
    int scorei = minimax(new_state, !maximizing_Player, player);
    if(scorei > max_score)
      max_score = scorei;
    if(scorei < min_score)
      min_score = scorei;
...
char AI_minimax(pstate state){
  ...
  for(int i = 0; i < NB_COLORS; i++){
    ...
    int color_score = minimax(state_cpy, false, state->curr_player);
