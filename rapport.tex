...
\begin{qu}
    En premier lieu, pour obtenir une version plus efficace, nous avons décidé d'utiliser un algorithme récursif.
    C'est implémenté par la fonction \verb|recursive_propagation| (bof) du module \verb|board.c|.
    ...
    Pour stocker les cases à visiter, nous avons implémenté une structure de file basée sur une double liste (pour plus de détails, voir le module \verb|structures.c|).
    Pour vérifier que cette méthode fonctionne correctement, on compare le résultat obtenu avec celui de la question précédente.
\end{qu}
